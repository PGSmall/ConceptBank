...
# =========================================================
# 4. Dataset & Pseudo-Label Image Utilities
# =========================================================
...
def _extract_img_only(sample: dict) -> Image.Image:
    """提取图像并抛弃对 GT 掩码的任何依赖"""
    img = sample.get("img", sample.get("inputs", None))
    if img is None:
        raise RuntimeError(f"Cannot extract img from sample keys={list(sample.keys())}")
    ...
    if arr.ndim == 3 and arr.shape[2] == 3:
        ...
        return Image.fromarray(arr.transpose(1, 2, 0).astype(np.uint8)).convert("RGB")
...
# =========================================================
# 7. Main Builder (Pseudo-Label / Visual Extractor)
# =========================================================
@torch.inference_mode()
def build_one_user_bank(
    ...
    try: bg_idx = int(getattr(cfg.model, "bg_idx", 0))
    except Exception: bg_idx = 0

    try: bg_thr = float(getattr(cfg.model, "bg_thr", 0.0))
    except Exception: bg_thr = 0.0

    try: calib_per_class = int(getattr(cfg.model, "calib_per_class", 10))
    except Exception: calib_per_class = 10

    try: calib_max_class = int(getattr(cfg.model, "calib_max_class", 10))
    except Exception: calib_max_class = 10
    ...
    if is_main():
        print0(f"\n========== [USER] {user_name} ==========")
        print0(f"[INFO] Concept Bank without GT Mask")
    ...
    primary_names = [ph[0] for ph in phrases_per_class]

    if not global_text_layout_cache:
        out0 = model.backbone.forward_text([primary_names[0]], device=device)
        lf0, pm0, le0, layout_ = _align_language_outputs(out0, B=1)
    ...
    prim_lf_list, prim_pm_list, prim_le_list = [], [], []
    for i in range(0, C, 32):
        batch = primary_names[i:i+32]
        with amp_ctx:
            out = model.backbone.forward_text(batch, device=device)
        lf, pm, le, _ = _align_language_outputs(out, B=len(batch))
        prim_lf_list.append(lf.cpu())
        prim_pm_list.append(pm.cpu())
        prim_le_list.append(le.cpu())

    prim_lf = torch.cat(prim_lf_list, dim=0)
    prim_pm = torch.cat(prim_pm_list, dim=0)
    prim_le = torch.cat(prim_le_list, dim=0)

    @torch.inference_mode()
    def get_pseudo_masks(img: Image.Image, qids_to_check: List[int], threshold: float = 0.35, chunk_size: int = 32):
        found_masks = {}
        if not qids_to_check: return found_masks
        
        st = processor.set_image(img)
        H, W = img.height, img.width
        
        dtype_float = torch.float16 if device.type == "cuda" else torch.float32
        
        for i in range(0, len(qids_to_check), chunk_size):
            batch_qids = qids_to_check[i:i+chunk_size]
            B_sub = len(batch_qids)
            
            sub_lf = prim_lf[batch_qids].to(device, dtype=dtype_float)
            sub_pm = prim_pm[batch_qids].to(device).bool()
            sub_le = prim_le[batch_qids].to(device, dtype=dtype_float)
            
            raw_lf = to_raw_from_BTD(sub_lf, layout["language_features"])
            raw_pm = mask_to_raw_from_BT(sub_pm, layout["language_mask"])
            raw_le = to_raw_from_BTD(sub_le, layout["language_embeds"])
            
            st["backbone_out"].update({
                "language_features": raw_lf,
                "language_mask": raw_pm,
                "language_embeds": raw_le
            })
            
            find_stage = FindStage(
                img_ids=torch.zeros(B_sub, device=device, dtype=torch.long),
                text_ids=torch.arange(B_sub, device=device, dtype=torch.long),
                input_boxes=None, input_boxes_mask=None, input_boxes_label=None,
                input_points=None, input_points_mask=None,
            )
            
            try: geo_prompt = model._get_dummy_prompt(num_prompts=B_sub)
            except TypeError:
                try: geo_prompt = model._get_dummy_prompt(B_sub)
                except TypeError: geo_prompt = model._get_dummy_prompt()
            ...
            logits = outg["pred_logits"]
            if logits.ndim == 3 and logits.size(-1) == 1: logits = logits.squeeze(-1)
            scores = logits.sigmoid() # [B_sub, N]
            pres = outg["presence_logit_dec"].sigmoid().view(-1)
            
            pms = outg["pred_masks"]
            if pms.ndim == 5 and pms.size(2) == 1: pms = pms.squeeze(2)
            
            for b in range(B_sub):
                qid = batch_qids[b]
                s = scores[b]
                
                max_s, max_idx = s.max(dim=0)
                final_score = max_s * pres[b]
                
                if final_score > threshold:
                    mask_hf = pms[b, max_idx:max_idx+1].unsqueeze(0)
                    mask_up = interpolate(mask_hf.float(), (H, W), mode="bilinear", align_corners=False).sigmoid()
                    m_bin = (mask_up.squeeze().cpu().numpy() > 0.5).astype(np.uint8)
                    
                    if m_bin.sum() > 20 or threshold < 0:
                        found_masks[qid] = m_bin
                        
        return found_masks

    # =====================================================
    # Stage I: Pass 1 Prototype Collection (Zero-Shot)
    # =====================================================
    _stage_cleanup_cuda()
    ...
    pseudo_thr = getattr(args, 'pseudo_thr', 0.35)

    for idx in it1:
        if (cnt_local_cpu >= cnt_limit_cpu).all(): break
        try:
            sample = dataset[idx]
            img = _extract_img_only(sample)
        except Exception:
            continue

        qids_to_check = [q for q in range(C) if cnt_local_cpu[q] < cnt_limit_cpu[q]]
        if not qids_to_check: continue

        found_masks = get_pseudo_masks(img, qids_to_check, threshold=pseudo_thr)
        if not found_masks: continue
        
        img_np = np.asarray(img)
        found_qids = sorted(list(found_masks.keys()), key=lambda q: (int(cnt_local_cpu[q]), q))

        for qid in found_qids:
            if cnt_local_cpu[qid] >= cnt_limit_cpu[qid]: continue
            
            m_np = found_masks[qid]
            views, _ = make_crop_views_from_np(
                img_np, m_np, class_id=1,
            ...
            try: e = compute_crop_embedding_device(views)
            except Exception: continue

            if D_local == 0:
                D_local = int(e.numel())
                if D_local <= 0: continue
    ...
    if D_global <= 0:
        ...
        if is_main():
            print0(f"[{user_name}] Pass1: No valid pseudo-labeled crops found; falling back to pure text.")
        prof["stage1_crops"] = 0
        ...
        if is_main():
            print0(f"[{user_name}] Pass1 Done. Valid visual protos via Pseudo-Labeling: {int(proto_valid_cpu.sum())}/{C}")
    ...
    # =====================================================
    # Stage II: Pass 2 Multi-epoch Fill (Pseudo-Label Mining)
    # =====================================================
    ...
    def heap_size(c): return len(heaps[c])

    def all_full():
        for c in range(C):
            if target[c] > 0 and heap_size(c) < target[c] and proto_valid_cpu[c]: return False
        return True
    ...
    if D_global > 0 and bool(proto_valid_cpu.any()):
        for epoch in range(args.pass2_max_epochs):
            ...
            for idx in it2:
                ...
                qids_to_check = [q for q in range(C) if target[q] > 0 and proto_valid_cpu[q] and heap_size(q) < target[q]]
                if not qids_to_check: continue
                ...
                for qid, m_np in found_masks.items():
                    if heap_size(qid) >= target[qid]: continue

                    key = (int(idx), int(qid), int(args.use_context_view), int(args.use_masked_view), float(args.pad_ratio), int(args.min_crop_size))
                ...
                for qid, sc in zip(qids, scores):
                    tie += 1
                    item = (float(sc), tie, int(idx), int(qid))
                    h = heaps[qid]
                    if len(h) < target[qid]: heapq.heappush(h, item)
                    elif float(sc) > h[0][0]: heapq.heapreplace(h, item)

    local_cands = [
        [(float(s), int(i), int(qid_stored)) for (s, _, i, qid_stored) in sorted(heaps[c], key=lambda x: x[0], reverse=True)]
        for c in range(C)
    ]
    ...
    if is_main():
        ...
        for c in range(C):
            ...
            selected_meta[c] = [(idx, qid_stored, float(score)) for (score, idx, qid_stored) in top]
    ...
    ddp_barrier()
    try: prof["stage2_crops"] = int(sum(len(selected_meta[c]) for c in range(C)))
    except Exception: prof["stage2_crops"] = 0

    try: del sum_local, cnt_local
    ...
    # =====================================================
    # Stage III: Text Prompt Encoding + Scoring/Fusion
    # =====================================================
    ...
    @torch.inference_mode()
    def _ctx_soft_prob_map(ctx_view: Image.Image, H: int, W: int, lf_raw: torch.Tensor, pm_raw: torch.Tensor, le_raw: torch.Tensor) -> torch.Tensor:
        ...
        try: geo_prompt_1 = model._get_dummy_prompt(num_prompts=1)
        except TypeError:
            try: geo_prompt_1 = model._get_dummy_prompt(1)
            except TypeError: geo_prompt_1 = model._get_dummy_prompt()
    ...
    @torch.inference_mode()
    def _estimate_bg_thr_for_class_dice(crops_c: List[Tuple[List[Image.Image], Image.Image, torch.Tensor]], lf_raw: torch.Tensor, pm_raw: torch.Tensor, le_raw: torch.Tensor) -> float:
        mode = str(getattr(args, "bg_thr_mode", "dice")).lower()
        if mode == "none" or not crops_c: return 0.0

        crops_use = crops_c[:16]
        pairs: List[Tuple[torch.Tensor, torch.Tensor]] = []
        for (_, ctx_view, gt_mask_cpu) in crops_use:
            gt = gt_mask_cpu.detach().cpu()
            if gt.ndim != 2: gt = gt.squeeze()
            H, W = int(gt.shape[-2]), int(gt.shape[-1])
            prob_hw = _ctx_soft_prob_map(ctx_view, H, W, lf_raw, pm_raw, le_raw)
            if prob_hw.shape != gt.shape:
                prob_hw = interpolate(prob_hw.unsqueeze(0).unsqueeze(0), (H, W), mode="bilinear", align_corners=False).squeeze(0).squeeze(0).cpu().float()
            pairs.append((prob_hw, gt > 0.5))

        if not pairs: return 0.0

        score_samples = []
        for (p, _) in pairs: score_samples.append(_rand_subsample_1d(p, 50000))
        samp = torch.cat(score_samples, dim=0)
        if samp.numel() == 0: return 0.0

        q_grid = torch.linspace(0.02, 0.98, steps=64)
        try: cand = torch.quantile(samp, q_grid).clamp(0.0, 1.0)
        except Exception: cand = torch.linspace(float(samp.min().item()), float(samp.max().item()), steps=64).clamp(0.0, 1.0)
        ...
        for t in cand.tolist():
            s = 0.0
            for (p, gt) in pairs: s += _dice_from_prob(p, gt, t)
            s /= float(len(pairs))
    ...
    def _score_all_prompts_for_class(views, gt_mask, lf, pm, le, B):
        ...
        try: geo_prompt = model._get_dummy_prompt(num_prompts=B)
        except TypeError: geo_prompt = model._get_dummy_prompt(B)
    ...
    for c in itc:
        ...
        for (idx, qid_stored, _) in sel_list:
            try:
                sample = dataset[idx]
                img = _extract_img_only(sample)
            except Exception: continue

            found_masks = get_pseudo_masks(img, [c], threshold=-1.0)
            m_np = found_masks.get(c, None)
            if m_np is None: continue

            img_np0 = np.asarray(img)
            box = _square_box_from_mask(m_np, 1, args.pad_ratio, args.min_crop_size)
            if box is None: continue
            
            x1, y1, x2, y2 = box
            crop_np = img_np0[y1 : y2 + 1, x1 : x2 + 1].copy()
            m_crop = (m_np[y1 : y2 + 1, x1 : x2 + 1] == 1).astype(np.uint8)

            ctx_view = Image.fromarray(crop_np)
            views_sel: List[Image.Image] = []
            if args.use_context_view: views_sel.append(ctx_view)
            if args.use_masked_view:
                bg = np.full_like(crop_np, 127, dtype=np.uint8)
                masked = np.where(m_crop[..., None].astype(bool), crop_np, bg)
                views_sel.append(Image.fromarray(masked))

            if not views_sel: views_sel = [ctx_view]
            crops_c.append((views_sel, ctx_view, torch.from_numpy(m_crop.astype(np.float32))))
        ...
        if len(crops_c) == 0:
            ...
            if torch.cuda.is_available() and (c % 8 == 0): _stage_cleanup_cuda()
            continue
        ...
        if c == bg_idx:
            ...
            if str(args.bg_thr_mode).lower() == "none":
                ...
                try:
                    ...
                    if "out of memory" in str(e).lower(): _stage_cleanup_cuda()
                    local_bg_thr[c] = float(bg_thr)
                except Exception:
                    local_bg_thr[c] = float(bg_thr)
    ...
    if dist_is_on():
        ...
        if is_main():
            ...
            for d in gathered_chosen:
                if isinstance(d, dict): merged.update(d)
    ...
    if is_main():
        ...
        bg_cfg = {
            ...
            "calib_view": "pseudo_labeling_auto_crop",
        ...
        user_bank = {
            ...
            "meta": {
                "type": "Zero-Shot Pseudo-Label Visual Concept SubBank",
                "bg_idx": int(bg_idx),
                ...
                "dsid_to_qid": {i: i for i in range(C)},
                "world_size": world,
                "bg_thr_cfg": bg_cfg,
                "profile": profile_min,
            },
...
def main():
    ...
    parser.add_argument("--bg_thr_mode", type=str, default="none", choices=["dice", "none"])

    parser.add_argument("--pseudo_thr", type=float, default=0.3)
    ...
    if is_main():
        ...
        bank = {
            ...
            "version": "v1.0 (Zero-Shot Visual Pseudo-Label Mode)",
            "users": users_out,
