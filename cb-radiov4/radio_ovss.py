...
def _bank_bg_thr_mode(subbank: dict) -> str:
    m = subbank.get("bg_thr_mode", None)
    if isinstance(m, str) and m.strip():
        return m.strip().lower()
    meta = subbank.get("meta", {})
    cfg = meta.get("bg_thr_cfg", {}) if isinstance(meta, dict) else {}
    m2 = cfg.get("mode", None) if isinstance(cfg, dict) else None
    if isinstance(m2, str) and m2.strip():
        return m2.strip().lower()
    return "auto"
...
class SAM3_OVSS(BaseSegmentor):
    def __init__(
        ...
        topk_inst: int = 30,
        prompt_chunk_size: int = 48,
        checkpoint_path: str = "pretrained/sam3/sam3.pt",
        bpe_path: str = "pretrained/clip/bpe_simple_vocab_16e6.txt.gz",
        radio_model_version: str = "pretrained/radio/c-radio_v4-h_half.pth.tar",
        ...
        **kwargs,
        ...
        if not skip_radio:
            radio_model = load_radio_model(radio_model_version, device=device)
            self.model = replace_sam3_encoder(self.model, radio_model, device=device)
        else:
        ...
        if skip_radio:
            ...
            self.processor = create_sam3_radio_processor(
            self.model,
        ...
        self.bg_thr_mode = _bank_bg_thr_mode(subbank)
        ...
        print(
            ...
            f"bank_bg_thr_mode='{self.bg_thr_mode}' | use_pc={self.use_bg_thr_per_class} | bg_thr={self.bg_thr}"
        )
